...
from models import Game as GameModel, GameType, Player, Round, RoundInfo, User

from ..base_game import GameSession


class GameRepositoryWriter:
    ...
    def _ensure_round_info(self, game_id: int, rounds_by_num, round_info_entries):
        ...
        existing_info = (
            self.db.query(RoundInfo).filter(RoundInfo.round_id.in_(round_ids)).all() if round_ids else []
        )
